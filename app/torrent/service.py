...
class TorrentService:
    ...
    def find_existing_finished_content(self, user_id: int, target_name: str | None) -> Path | None:
        """Return existing finished file/folder for user when target name is present."""
        if target_name is None:
            return None

        normalized_target_name = target_name.strip()
        if not normalized_target_name:
            return None

        if "/" in normalized_target_name or "\\" in normalized_target_name:
            return None

        target_segment = Path(normalized_target_name)
        if target_segment.name != normalized_target_name or normalized_target_name in {".", ".."}:
            return None

        user_root = (self._finished_downloads_root / str(user_id)).resolve()
        try:
            candidate_path = (user_root / normalized_target_name).resolve()
        except OSError:
            return None

        if candidate_path != user_root and user_root not in candidate_path.parents:
            return None
